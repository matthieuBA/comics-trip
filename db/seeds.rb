...
books.each_with_index do |book, i|
    b=Book.create(title:book.title, author:book.authors, genre:book.categories, isbn:book.isbn, picture:book.image_link(:zoom => 4), abstract:book.description, extract:book.description)
    bc=BookCard.create(user_id: User.all.sample.id, book_id: Book.last.id, price: rand(100), to_sell: sell[rand(3)], book_condition: conditions[rand(3)], book_picture:Book.last.picture, review:Book.last.abstract)
